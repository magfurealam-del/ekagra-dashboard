...
export default function CallCenterPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    if (search) {
      q = q.or(`patient_name.ilike.%${search}%,mobile_normalised.ilike.%${search}%,patient_id_raw.ilike.%${search}%`);
    }
    ...
    if (filters.doctor) q = q.ilike("doctor_name_raw", `%${filters.doctor}%`);
  ...
  const saveEdit = async (id: number) => {
    ...
    if (res.ok) {
      setLogs(prev => prev.map(l => l.id === id ? { ...l, ...editValues } as CallCenterLog : l));
      setEditingId(null);
    }
    setSaving(false);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <span className="text-sm text-gray-500 self-center">{total.toLocaleString()} records</span>
          <Link href="/call-center/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">
            + New Lead
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border p-4 space-y-3">
        <input
          type="text" placeholder="Search by name, phone, or patient ID…"
          className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          value={search} onChange={e => { setSearch(e.target.value); setPage(0); }}
        />
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
          <input type="date" className="border rounded px-2 py-1.5 text-xs" placeholder="From" value={filters.dateFrom} onChange={e => handleFilterChange("dateFrom", e.target.value)} />
          <input type="date" className="border rounded px-2 py-1.5 text-xs" placeholder="To" value={filters.dateTo} onChange={e => handleFilterChange("dateTo", e.target.value)} />
        ...
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer">
            <input type="checkbox" checked={filters.noShowOnly} onChange={e => handleFilterChange("noShowOnly", e.target.checked)} className="rounded" />
            No Shows Only
          </label>
          <label className="flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer">
            <input type="checkbox" checked={filters.fbOnly} onChange={e => handleFilterChange("fbOnly", e.target.checked)} className="rounded" />
            Facebook Only
          </label>
          <button onClick={() => { setFilters({ dateFrom: "", dateTo: "", appointmentStatus: "", finalStatus: "", agent: "", doctor: "", leadCategory: "", source: "", noShowOnly: false, fbOnly: false }); setSearch(""); setPage(0); }} className="text-xs text-blue-600 hover:underline">
            Clear filters
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={15} className="px-3 py-8 text-center text-gray-400">No records found</td></tr>
              ) : (
                logs.map(log => {
                  const isEditing = editingId === log.id;
                  return (
                    <tr key={log.id} className={`hover:bg-gray-50 ${isEditing ? "bg-blue-50" : ""}`}>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{formatDate(log.log_date)}</td>
                      <td className="px-3 py-2">
                        <Link href={`/call-center/${log.id}`} className="font-medium text-blue-700 hover:underline text-xs">
                          {log.patient_name || "—"}
                        </Link>
                      ...
                      <td className="px-3 py-2 text-xs max-w-[120px] truncate">
                        ...
                        ) : (log.doctor_name_raw?.replace("Dr. ", "Dr ") || "—")}
                      </td>
                      <td className="px-3 py-2 text-xs whitespace-nowrap">{formatDate(log.appointment_date || log.appointment_datetime?.split("T")[0] || null)}</td>
                      <td className="px-3 py-2 text-xs">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">
                        {isEditing ? (
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => setEditingId(null)} className="bg-gray-200 text-gray-700 px-2 py-0.5 rounded text-xs hover:bg-gray-300">Cancel</button>
                          </div>
                        ) : (
                          <div className="flex gap-1">
